// ==== DADOS (exato do seu quadro de sexta/sábado/domingo) ====
// Mantive o conteúdo do seu COLUMNS; você pode adicionar "startAt" (segundos ou "mm:ss") em itens específicos
const COLUMNS = [
  {
    id: "sexta",
    title: "SEXTA-FEIRA",
    items: [
      { type: "section", id: "sx-sec-1", time: "19:30", label: "PÁTIO", duration: "30:00", pagHead: "PAG" },

      { id: "sx-01", time: "", title: "Boa noite amigo boa noite irmão", tone: "D", page: 14, url: "audio/boa noite amigo.mp3" },
      { id: "sx-02", time: "", title: "Celebrai com jubilo", tone: "D", page: 64, url: "audio/Celebrai com jubilo.mp3" },
      { id: "sx-03", time: "", title: "Reunidos Aqui", tone: "D", page: 13, url: "" },
      { id: "sx-04", time: "", title: "A alegria está no coração", tone: "D", page: 5, url: "audio/exmeplo.mp3" },
      { id: "sx-05", time: "", title: "Cantemos Aleluia", tone: "D", page: "", url: "" },
      { id: "sx-06", time: "", title: "Se o meu povo que se chama pelo meu nome", tone: "D", page: 61, url: "" },
      { id: "sx-07", time: "", title: "Eis que venho sem demora", tone: "D", page: 10, url: "" },

      { type: "note", id: "sx-note-1", label: "SEJAM BEM VINDOS AO 8º ECC DE SERRINHA" },

      { id: "sx-08", time: "", title: "Só o poder de Deus", tone: "D", page: 11, url: "" },
      { id: "sx-09", time: "", title: "Batalhar não é batalha", tone: "D", page: 64, url: "" },
      { id: "sx-10", time: "", title: "Jacó Segurou o Anjo", tone: "D", page: 62, url: "" },

      { type: "section", id: "sx-sec-2", time: "20:00", label: "do PÁTIO para Plenário", duration: "10:00" },
      { id: "sx-11", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "sx-sec-3", time: "20:10", label: "PLENÁRIO", duration: "10:00" },
      { id: "sx-12", time: "", title: "Foi bom conhecer você (Amigo é uma alegria ter)", tone: "D", page: 6, url: "audio/Foi Bom Conhecer Voce.m4a" },
    ],
  },

  {
    id: "sabado",
    title: "SÁBADO",
    items: [
      { type: "section", id: "sa-sec-1", time: "08:00", label: "PÁTIO", duration: "30:00", pagHead: "PAG" },

      { id: "sa-01", time: "", title: "Bom dia amigo bom dia irmão", tone: "D", page: 14, url: "" },
      { id: "sa-02", time: "", title: "Celebrai com júbilo", tone: "D", page: 64, url: "" },
      { id: "sa-03", time: "", title: "Os que confiam no Senhor", tone: "D", page: 49, url: "" },
      { id: "sa-14", time: "", title: "Desperta", tone: "Dm", page: 15, url: "" },
      { id: "sa-15", time: "", title: "Aqueles que tem sede busca", tone: "Dm", page: 51, url: "" },
      { id: "sa-16", time: "", title: "Aleluia", tone: "D", page: 48, url: "" },
      { id: "sa-17", time: "", title: "A minha vida é do mestre", tone: "D", page: 54, url: "" },
      { id: "sa-18", time: "", title: "Ele não desiste de você", tone: "D", page: 28, url: "" },
      { id: "sa-19", time: "", title: "Quero louvar-te", tone: "D", page: 43, url: "" },
      { id: "sa-20", time: "", title: "Como Zaqueu", tone: "D", page: 39, url: "" },

      { type: "section", id: "sa-sec-2", time: "08:20", label: "do PÁTIO para PLENÁRIO", duration: "" },
      { id: "sa-05", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "sa-sec-2", time: "08:30", label: "PLENÁRIO (Oração da Manhã)", duration: "" },

      { id: "sa-06", time: "", title: "Com Cristo no meu barco", tone: "D", url: "audio/Com Cristo No Meu Barco.m4a" },

      { type: "section", id: "sa-sec-2", time: "09:20", label: "do PÁTIO para CÍRCULO", duration: "" },
      { id: "sa-07", time: "", title: "Conte a Deus", tone: "D", page: 38, url: "audio/Conte A Deus.m4a" },

      { type: "section", id: "sa-sec-2", time: "10:00", label: "do CÍRCULO  para PÁTIO", duration: "" },
      { id: "sa-08", time: "", title: "Com muito louvor (Deus vai na frente)", tone: "A", page: 22, url: "" },

      { type: "section", id: "sa-sec-2", time: "10:10", label: "PÁTIO", duration: "" },
      { id: "sa-09", time: "", title: "Eu quero é Deus", tone: "D", page: 23, url: "" },
      { id: "sa-10", time: "", title: "Ele vem (O tempo de cantar chegou)", tone: "D", page: 32, url: "" },
      { id: "sa-11", time: "", title: "Eu sou de Jesus)", tone: "C", page: 45, url: "" },
      { id: "sa-12", time: "", title: "Jesus Virá (Num dia lindo)", tone: "D", page: 65, url: "" },
      { id: "sa-13", time: "", title: "Jeová Senhor nosso", tone: "Dm", page: 6, url: "" },
      { id: "sa-14", time: "", title: "Há uma unção", tone: "F", page: 29, url: "" },
      { id: "sa-15", time: "", title: "Nosso general", tone: "Dm", page: 57, url: "" },

      { type: "section", id: "sa-sec-2", time: "10:30", label: "do PÁTIO para PLENÁRIO", duration: "" },
      { id: "sa-16", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "sa-sec-2", time: "11:40", label: "PLENÁRIO", duration: "" },
      { id: "sa-17", time: "", title: "Utopia", tone: "E", page: 52, url: "audio/utopia.mpa" },

      { type: "section", id: "sa-sec-2", time: "11:45", label: "do PLENÁRIO para CONVERSA A DOIS", duration: "" },
      { id: "sa-18", time: "", title: "Só o poder de Deus", tone: "D", page: 11, url: "" },

      { type: "section", id: "sa-sec-2", time: "12:10", label: "CONVERSA A DOIS para ALMOÇO", duration: "" },
      { id: "sa-18", time: "", title: "Ha uma unção", tone: "F", page: 29, url: "" },
      { id: "sa-19", time: "", title: "Venho senhor a minha vida oferecer", tone: "F", page: 15, url: "" },

      { type: "section", id: "sa-sec-2", time: "12:20", label: "ALMOÇO", duration: "" },
      { id: "sa-18", time: "", title: "Ao Senhor agradecemos essa nossa refeição", tone: "", url: "" },

      { type: "section", id: "sa-sec-2", time: "13:40", label: "ALMOÇO para CÍRCULO", duration: "" },
      { id: "sa-18", time: "", title: "Chuva de poder", tone: "D", page: 47, url: "" },
      { id: "sa-19", time: "", title: "Videira", tone: "D", page: 19, url: "" },

      { type: "section", id: "sa-sec-2", time: "14:05", label: "do CÍRCULO para PLENÁRIO", duration: "" },
      { id: "sa-18", time: "", title: "O tempo não pode apagar", tone: "G", page: 26, url: "" },

      { type: "section", id: "sa-sec-2", time: "15:40", label: "do PLENÁRIO para CÍRCULO", duration: "" },
      { id: "sa-18", time: "", title: "Como vai a sua vida", tone: "", page: 46, url: "" },

      { type: "section", id: "sa-sec-2", time: "16:05", label: "CONVERSA A DOIS para PÁTIO", duration: "" },
      { id: "sa-18", time: "", title: "Nós precisamos de Cristo", tone: "D", page: 63, url: "" },

      { type: "section", id: "sa-sec-2", time: "16:10", label: "PÁTIO", duration: "" },
      { id: "sa-09", time: "", title: "Vem desfrutar do amor de Deus", tone: "D", page: 18, url: "" },
      { id: "sa-10", time: "", title: "A ti Senhor", tone: "D", page: 5, url: "" },
      { id: "sa-11", time: "", title: "Ouve-se um júbilo (Leão de Judá)", tone: "D", page: 7, url: "" },
      { id: "sa-12", time: "", title: "Deus forte como Jeová", tone: "D", page: 10, url: "" },
      { id: "sa-13", time: "", title: "Meu barquinho", tone: "D", page: 40, url: "" },
      { id: "sa-14", time: "", title: "Quero que valorize (Você tem valor)", tone: "D", page: 51, url: "" },
      { id: "sa-15", time: "", title: "Raridade (começa do refrão)", tone: "D", page: 27, url: "" },

      { type: "section", id: "sa-sec-2", time: "16:25", label: "do PÁTIO para PLENÁRIO", duration: "" },
      { id: "sa-18", time: "", title: "Somos todos nós (Entra cantando)", tone: "D", page: 8, url: "" },

      { type: "section", id: "sa-sec-2", time: "16:35", label: "PLENÁRIO", duration: "" },
      { id: "sa-18", time: "", title: "POROPOPÓ (Ensaio do maestro)", tone: "", page: 59, url: "" },

      { type: "section", id: "sa-sec-2", time: "17:30", label: "do PLENÁRIO para CÍRCULO ", duration: "" },
      { id: "sa-18", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "sa-sec-2", time: "18:05", label: "do CÍRCULO para PLENÁRIO", duration: "" },
      { id: "sa-18", time: "", title: "Aquele que está feliz", tone: "", page: 21, url: "" },

      { type: "section", id: "sa-sec-2", time: "19:20", label: "do PLENÁRIO para JANTAR", duration: "" },
      { id: "sa-18", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "sa-sec-2", time: "20:50", label: "do JANTAR para PLENÁRIO", duration: "" },
      { id: "sa-18", time: "", title: "Somos todos nós", tone: "D", page: 8, url: "" },
    ],
  },

  {
    id: "domingo",
    title: "DOMINGO",
    items: [
      { type: "section", id: "dg-sec-1", time: "08:15", label: "QUADRA", duration: "30:00", pagHead: "PAG" },
      { id: "dg-01", time: "", title: "Bom dia amigo, bom dia irmão", tone: "D", page: 14, url: "" },
      { id: "dg-02", time: "", title: "Chuva de poder", tone: "D", page: 47, url: "" },
      { id: "dg-03", time: "", title: "Videira", tone: "D", page: 19, url: "" },
      { id: "dg-03", time: "", title: "Nós precisamos de Cristo", tone: "D", page: 63, url: "" },
      { id: "dg-03", time: "", title: "Como é bom ser cristão, como é bom", tone: "C", url: "" },
      { id: "dg-03", time: "", title: "Noites traiçoeiras", tone: "D", page: 25, url: "" },

      { type: "section", id: "dg-sec-2", time: "08:25", label: "da Quadra para PÁTIO", duration: "20:00" },
      { id: "dg-04", time: "", title: "Somos todos nós (variando o ritmo)", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "08:35", label: "PÁTIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Poderoso Deus", tone: "D", page: 30, url: "" },
      { id: "dg-05", time: "", title: "Eu vejo uma pequena nuvem", tone: "Dm", page: 31, url: "" },
      { id: "dg-05", time: "", title: "MUSICA DO BISCOITO", tone: "", url: "audio/musica do biscoito.mp3" },

      { type: "section", id: "dg-sec-1", time: "08:35", label: "do PÁTIO para PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós (Variando o ritmo)", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "08:45", label: "PLENÁRIO (Oração da Manhã)", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Deus está aqui", tone: "D", url: "" },
      { id: "dg-05", time: "", title: "Com amor no lar", tone: "F", url: "" },

      { type: "section", id: "dg-sec-1", time: "09:45", label: "PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Jesus Cristo mudou meu viver", tone: "D", page: 58, url: "" },

      { type: "section", id: "dg-sec-1", time: "09:50", label: "do PLENÁRIO para PIRA", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Nenhuma condenação há", tone: "D", page: 33, url: "" },

      { type: "section", id: "dg-sec-1", time: "10:00", label: "PIRA", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Sou um milagre / Nunca houve noite", tone: "D", page: 20, url: "" },

      { type: "section", id: "dg-sec-1", time: "10:25", label: "da PIRA para PÁTIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "10:30", label: "PÁTIO", duration: "30:00", pagHead: "PAG" },
      { id: "dg-01", time: "", title: "Recebi um novo coração", tone: "D", page: 60, url: "" },
      { id: "dg-02", time: "", title: "Rompendo em fé", tone: "D", page: 56, url: "" },
      { id: "dg-03", time: "", title: "Venho senhor a minha vida oferecer", tone: "D", page: 15, url: "" },
      { id: "dg-03", time: "", title: "Ele vem (O tempo de cantar chegou)", tone: "D", page: 32, url: "" },
      { id: "dg-03", time: "", title: "Jesus em sua presença", tone: "D", page: 63, url: "" },
      { id: "dg-03", time: "", title: "Solta o cabo da nau", tone: "D", page: 16, url: "" },

      { type: "section", id: "dg-sec-1", time: "10:50", label: "do PÁTIO para PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "11:00", label: "PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "POROPOPÓ (Ensaio do Maestro)", tone: "", url: "" },

      { type: "section", id: "dg-sec-1", time: "11:55", label: "do PLENÁRIO para ALMOÇO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "12:05", label: "do PLENÁRIO para ALMOÇO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Ao Senhor agradecemos essa nossa refeição", tone: "", url: "" },
      { id: "dg-05", time: "", title: "Pensando que cachaça é agua / Batida de Coco", tone: "", url: "" },
      { id: "dg-05", time: "", title: "A turma da cozinha é animada", tone: "", url: "audio/a turma da cozinha e animada.mp3" },
      { id: "dg-05", time: "", title: "Musica do Parabens para você", tone: "", url: "" },

      { type: "section", id: "dg-sec-1", time: "13:35", label: "BEIJINHO DOCE", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Beijinho Doce", tone: "", url: "" },

      { type: "section", id: "dg-sec-1", time: "13:55", label: "do BEIJINHO DOCE para PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós (Entra cantando)", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "14:25", label: "PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Apresentação do Maestro", tone: "", url: "" },

      { type: "section", id: "dg-sec-1", time: "15:10", label: "do PLENÁRIO para CÍRCULO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Deus forte como Jeová", tone: "", page: 10, url: "" },
      { id: "dg-05", time: "", title: "Ouve-se um júbilo (Leão de Judá)", tone: "", page: 7, url: "" },

      { type: "section", id: "dg-sec-1", time: "15:35", label: "do CÍRCULO PARA PÁTIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Casa de benção", tone: "", page: 36, url: "" },

      { type: "section", id: "dg-sec-1", time: "15:40", label: "PÁTIO", duration: "30:00", pagHead: "PAG" },
      { id: "dg-01", time: "", title: "Na minha vida", tone: "D", page: 62, url: "" },
      { id: "dg-02", time: "", title: "Viva a fé", tone: "D", page: 11, url: "" },
      { id: "dg-03", time: "", title: "Aquele que está feliz", tone: "D", page: 21, url: "" },
      { id: "dg-03", time: "", title: "Os que confiam no Senhor", tone: "D", page: 49, url: "" },
      { id: "dg-03", time: "", title: "Em adoração", tone: "D", page: 24, url: "" },
      { id: "dg-03", time: "", title: "Deus de promessa", tone: "D", page: 41, url: "" },

      { type: "section", id: "dg-sec-1", time: "16:00", label: "do PÁTIO para PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Somo todos nós (Entra cantando)", tone: "D", page: 8, url: "" },

      { type: "section", id: "dg-sec-1", time: "16:10", label: "PLENÁRIO", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "O céu escureceu (falsa despedida)", tone: "Am", url: "audio/falsa-despedida.mp3" },

      { type: "section", id: "dg-sec-1", time: "20:50", label: "PLENÁRIO (Apresentação das Equipes)", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Há uma luz", tone: "D", page: 9, url: "" },

      { type: "section", id: "dg-sec-1", time: "20:50", label: "PLENÁRIO (Caminho de velas)", duration: "", pagHead: "PAG" },
      { id: "dg-05", time: "", title: "Vaso novo", tone: "D", page: 4, url: "" },
    ],
  },
];

// ======= ELEMENTOS BÁSICOS =======
const gridEl = document.getElementById("grid");
const rowById = new Map();

// ======= LOGIN / APP =======
const appMain = document.querySelector("main.app");
const loginWrap = document.getElementById("login");
const phoneInput = document.getElementById("phoneInput");
const btnLogin = document.getElementById("btnLogin");
const loginMsg = document.getElementById("loginMsg");

let allowedPhones = [];

// ======= CONTROLE DE SESSÃO (10 min ou fechar página) =======
let sessionTimer = null;

function startSessionTimer() {
  clearTimeout(sessionTimer);
  sessionTimer = setTimeout(() => {
    logout();
    alert("Sua sessão expirou após 10 minutos.");
  }, 10 * 60 * 1000);
}

function logout() {
  try { clearTimeout(sessionTimer); } catch {}
  sessionTimer = null;
  localStorage.removeItem("authPhone");
  if (appMain) appMain.classList.add("hidden");
  if (loginWrap) loginWrap.classList.remove("hidden");
  if (loginMsg) loginMsg.textContent = "Sessão encerrada. Faça login novamente.";
}

// encerra sessão ao fechar/atualizar a página
window.addEventListener("beforeunload", () => {
  // Só limpar o storage; alterações no DOM aqui não fazem efeito prático
  localStorage.removeItem("authPhone");
});

// ======= FUNÇÕES AUX =======
function isAuthenticated() {
  return !!localStorage.getItem("authPhone");
}
function showApp() {
  loginWrap?.classList.add("hidden");
  appMain?.classList.remove("hidden");
}
function showLogin() {
  appMain?.classList.add("hidden");
  loginWrap?.classList.remove("hidden");
}
function onlyDigits(s) {
  return (s || "").replace(/\D+/g, "");
}
function parseStartAt(v) {
  // aceita número (segundos) ou "mm:ss"
  if (v == null || v === "") return 0;
  if (typeof v === "number") return v;
  if (typeof v === "string" && /^\d+:\d{2}$/.test(v)) {
    const [m, s] = v.split(":").map(n => parseInt(n, 10) || 0);
    return (m * 60) + s;
  }
  const n = Number(v);
  return isNaN(n) ? 0 : n;
}
function resolveUrl(u) {
  if (!u) return "";
  if (/^https?:\/\//i.test(u)) return u; // absoluta
  return new URL(u.replace(/^\/+/, ""), window.location.href).toString(); // relativa ao index.html
}

// ==== AUTO-RENEW DA SESSÃO (renova a cada interação útil) ====
(function attachSessionAutoRenew() {
  const renew = () => {
    if (document.visibilityState === "hidden") return;
    if (isAuthenticated()) startSessionTimer();
  };
  const EVENTS = [
    "click","keydown","input","play","pause","ended","touchstart","visibilitychange"
  ];
  EVENTS.forEach(evt => document.addEventListener(evt, renew, { passive: true }));
})();

// ======= RENDER =======
function render() {
  gridEl.innerHTML = "";
  COLUMNS.forEach((col) => {
    const colEl = document.createElement("section");
    colEl.className = "column";

    const head = document.createElement("div");
    head.className = "col-header";
    head.textContent = col.title;
    colEl.appendChild(head);

    col.items.forEach((it, idx) => {
      if (it.type === "section") {
        const pagHeadHTML = (idx === 0 && it.pagHead)
          ? `<span class="pag-head">${it.pagHead}</span>` : "";
        const durationHTML = it.duration
          ? `<span class="duration-badge">${it.duration}</span>` : "";

        const s = document.createElement("div");
        s.className = "section-row";
        s.innerHTML = `
          <div class="cell time">${it.time || ""}</div>
          <div class="cell title title--section title">${it.label}</div>
          <div class="cell right">${durationHTML || ""}</div>
          <div class="cell right">${pagHeadHTML}</div>
        `;
        colEl.appendChild(s);
        return;
      }

      if (it.type === "note") {
        const n = document.createElement("div");
        n.className = "note-row";
        n.innerHTML = `<div class="cell title">${it.label}</div>`;
        colEl.appendChild(n);
        return;
      }

      // linha de música
      const r = document.createElement("div");
      r.className = "row";
      r.tabIndex = 0;
      r.setAttribute("data-id", it.id);

      // marca visualmente e desabilita eventos se não houver url
      if (!it.url) r.classList.add("no-url");

      r.innerHTML = `
        <div class="cell time">${it.time || ""}</div>
        <div class="cell title">${it.title}</div>
        <div class="cell right">${it.tone || ""}</div>
        <div class="cell right">${it.page ?? ""}</div>
      `;

      if (it.url) {
        r.addEventListener("click", () => openModal(it));
        r.addEventListener("keydown", (e) => {
          if (e.key === "Enter" || e.key === " ") {
            e.preventDefault();
            openModal(it);
          }
        });
      }

      rowById.set(it.id, r);
      colEl.appendChild(r);
    });

    gridEl.appendChild(colEl);
  });
}

// ======= PLAYER (modal) =======
const modal = document.getElementById("modal");
const btnClose = document.getElementById("btnClose");
const audio = document.getElementById("audio");
const btnPlayPause = document.getElementById("btnPlayPause");
const btnStop = document.getElementById("btnStop");
const btnMute = document.getElementById("btnMute");
const trackTitle = document.getElementById("trackTitle");
const trackMeta = document.getElementById("trackMeta");

let currentTrack = null;

function setPlayingRowHighlight(id, playing) {
  rowById.forEach((el) => el.classList.remove("playing"));
  if (playing && id && rowById.get(id)) {
    rowById.get(id).classList.add("playing");
  }
}

function openModal(track) {
  currentTrack = track;

  // preencher header/meta
  trackTitle.textContent = track.title;
  const metaParts = [];
  if (track.time) metaParts.push(`Horário: ${track.time}`);
  if (track.tone) metaParts.push(`Tonalidade: ${track.tone}`);
  if (track.page !== undefined && track.page !== "") metaParts.push(`Pág.: ${track.page}`);
  trackMeta.textContent = metaParts.join(" · ");

  // configurar áudio (suporta startAt)
  const startAt = parseStartAt(track.startAt);
  audio.src = resolveUrl(track.url);

  const onLoaded = () => {
    try { audio.currentTime = startAt; } catch {}
  };
  audio.removeEventListener("loadedmetadata", onLoaded);
  audio.addEventListener("loadedmetadata", onLoaded, { once: true });

  // abrir modal
  modal.classList.remove("hidden");
  modal.setAttribute("aria-hidden", "false");
  document.body.style.overflow = "hidden"; // trava scroll do fundo

  audio.play().then(() => {
    btnPlayPause.textContent = "Pausar";
    setPlayingRowHighlight(track.id, true);
  }).catch((e) => {
    console.error("Erro ao tocar:", e);
    btnPlayPause.textContent = "Reproduzir";
    setPlayingRowHighlight(null, false);
  });
}

function closeModal() {
  stopAudio();
  modal.classList.add("hidden");
  modal.setAttribute("aria-hidden", "true");
  document.body.style.overflow = ""; // libera scroll do fundo
}

function stopAudio() {
  audio.pause();
  try { audio.currentTime = 0; } catch {}
  btnPlayPause.textContent = "Reproduzir";
  setPlayingRowHighlight(currentTrack?.id, false);
}

btnClose?.addEventListener("click", closeModal);
modal?.addEventListener("click", (e) => { if (e.target === modal) closeModal(); });
document.addEventListener("keydown", (e) => { if (e.key === "Escape" && !modal.classList.contains("hidden")) closeModal(); });

btnPlayPause?.addEventListener("click", () => {
  if (audio.paused) {
    audio.play().catch(console.error);
  } else {
    audio.pause();
  }
});
btnStop?.addEventListener("click", stopAudio);
btnMute?.addEventListener("click", () => { audio.muted = !audio.muted; });

audio.addEventListener("play", () => {
  btnPlayPause.textContent = "Pausar";
  if (currentTrack) setPlayingRowHighlight(currentTrack.id, true);
});
audio.addEventListener("pause", () => {
  btnPlayPause.textContent = "Reproduzir";
});
audio.addEventListener("ended", () => {
  btnPlayPause.textContent = "Reproduzir";
  setPlayingRowHighlight(currentTrack?.id, false);
});

// ======= LOGIN FLOW =======
async function initAuth() {
  try {
    const resp = await fetch("phones.json", { cache: "no-store" });
    if (!resp.ok) throw new Error("Falha ao carregar phones.json");
    allowedPhones = await resp.json();

    if (isAuthenticated()) {
      showApp();
      render();
      startSessionTimer();
    } else {
      showLogin();
    }
  } catch (err) {
    console.error(err);
    showLogin();
    if (loginMsg) loginMsg.textContent = "Não foi possível carregar a lista de usuários.";
  }
}

function tryLogin() {
  if (!phoneInput) return;
  if (loginMsg) loginMsg.textContent = "";
  const raw = phoneInput.value.trim();
  const digits = onlyDigits(raw);

  if (!digits) {
    if (loginMsg) loginMsg.textContent = "Informe seu número (somente dígitos).";
    phoneInput.focus();
    return;
  }

  const exists = Array.isArray(allowedPhones) &&
                 allowedPhones.some(p => onlyDigits(p) === digits);

  if (exists) {
    localStorage.setItem("authPhone", digits);
    showApp();
    render();
    startSessionTimer();
  } else {
    if (loginMsg) loginMsg.textContent = "Número não encontrado. Verifique com seu coordenador.";
  }
}

// Eventos do formulário de login
btnLogin?.addEventListener("click", tryLogin);
phoneInput?.addEventListener("keydown", (e) => {
  if (e.key === "Enter") {
    e.preventDefault();
    tryLogin();
  }
});

// Inicializa autenticação ao carregar
initAuth();
